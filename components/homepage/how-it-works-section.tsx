        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
          {steps.map((s, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-8 rounded-3xl bg-[#F5F7F8] border border-[#DCE5E4] hover:border-[#0F5C5A] hover:shadow-xl transition-all space-y-5 flex flex-col justify-between relative group"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-4xl font-black text-[#0F5C5A]/30 group-hover:text-[#0F5C5A] transition-colors">
                      {s.number}
                    </span>
                    <div className="p-3 rounded-2xl bg-white border border-[#DCE5E4] text-[#0F5C5A] shadow-xs">
