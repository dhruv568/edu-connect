...
export function WhyLiveSection() {
  const featureBlocks: FeatureBlock[] = [
    ...
    {
      icon: <HeartHandshake className="h-6 w-6 text-amber-300" />,
  ...
  return (
    <section id="why-educonnects" className="py-16 sm:py-24 bg-[#073F3C] text-white font-sans border-b border-[#F2C14E]/20 relative overflow-hidden">
      {/* Background Lighting */}
      <div className="absolute top-0 right-1/4 w-96 h-96 bg-[#F2C14E]/10 rounded-full blur-[140px] pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 relative z-10">
        {/* Section Header */}
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          <span className="text-xs font-black uppercase tracking-widest text-[#F2C14E] bg-[#F2C14E]/15 px-3.5 py-1 rounded-full border border-[#F2C14E]/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          {featureBlocks.map((block, i) => (
            <div
              key={i}
              className="p-6 sm:p-7 rounded-3xl bg-[#0B4F4B]/80 border border-[#F2C14E]/30 shadow-xl hover:border-[#F2C14E] hover:scale-[1.02] transition-all duration-300 flex flex-col space-y-4"
            >
              <div className="w-12 h-12 rounded-2xl bg-[#F2C14E]/15 border border-[#F2C14E]/30 flex items-center justify-center text-[#F2C14E]">
                {block.icon}
              </div>
